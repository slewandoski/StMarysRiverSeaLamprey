...
library(dplyr)
library(raster)
load('output/sdmTMB_models.RData')
# general plots and diagnostics, can (should) be run for each model
# tidy(m3, conf.int = TRUE)
# tidy(m3, effects = "ran_pars", conf.int = TRUE)
# plot depth effect
# ggeffects::ggpredict(m3, terms="depth[0:40, by = 2]") |> plot()
...
# set.seed(19283)
# s <- simulate(m3, nsim = 1000, type = "mle-mvn")
...
predictions <- predict(m3, newdata = grid_yrs, return_tmb_object = TRUE)
...
# multi-page pdf"
pdf("plots/ar1st_idx_std.pdf", width = 15, height = 11)
print(p1)
print(p2)
print(p3)
print(p4)
